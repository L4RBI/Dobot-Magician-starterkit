...
def catmull_rom_chain(points: tuple, num_points: int) -> list:
    ...
    all_splines = (catmull_rom_spline(*pq, num_points)
                   for pq in point_quadruples)
    return flatten(all_splines)


def genCurve(nbPoints=10, smoothness=200, draw=False, zLevel=None):
    if draw and zLevel != None:
        ...
        x = np.random.uniform(180, 250, nbPoints)
        y = np.random.uniform(-200, 200, nbPoints)
        z = np.random.uniform(0, 60, nbPoints)
    POINTS: tuple = tuple(zip(x, y, z))  # Red points
    # Density of blue chain points between two red points
    NUM_POINTS: int = int(smoothness / nbPoints)

    chain_points: list = catmull_rom_chain(POINTS, NUM_POINTS)
    xchain = [chain_points[i][0] for i in range(len(chain_points))]
    ychain = [chain_points[i][1] for i in range(len(chain_points))]
    zchain = [chain_points[i][2] for i in range(len(chain_points))]
    fig1 = plt.figure(figsize=(4, 4))
    ax1 = fig1.add_subplot(111, projection="3d")
    ax1.scatter(x, y, z, c="black")
    ax1.plot(xchain, ychain, zchain, c="blue")
    plt.show()
    fig1.savefig('catMullCurve.png')
    return x, y, z, xchain, ychain, zchain
